...
def run_tests(output_format)
  ...
  it "shows #{output_format} view" do
    ...
    allow(knife_bmcs_server_show.identity_client).to receive(:list_compartments).and_return(double(data: [compartmentA]))
    allow(knife_bmcs_server_show.compute_client).to receive(:list_images).and_return(double(data: [image1]))
    allow(knife_bmcs_server_show.network_client).to receive(:list_vcns).and_return(double(data: [vcn1]))
